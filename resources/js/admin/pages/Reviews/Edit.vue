<template>
    <form class="row w-100 m-auto justify-content-center align-items-center" v-on:submit.prevent="submit">
        ...
        <div class="col-12">
            <button class="btn btn-outline-info" type="submit">Update review</button>
        </div>
    </form>
</template>
...
<script>
    export default {
        ...
        methods: {
            submit() {
                this.message = null;
                axios.put(`/admin/reviews/edit/${this.json.id}`, this.json)
                    .then(resp=>{
                        this.sendMessage("Success reviews update!")
                    }).catch((e)=>{
