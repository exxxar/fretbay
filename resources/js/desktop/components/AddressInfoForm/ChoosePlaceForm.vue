        ...
        <div class="row">
            ...
            <div class="col-12" v-if="postal">
                <ValidationProvider :name="address" vid="address" rules="required" v-slot="{ errors }">
                    <address-input v-model="listing[address]" class="mb-2" v-on:selected="selectAddress"
                                   :disabled="false" placeholder="City" required/>
                    <p class="mb-0" style="color:red;font-size:11px">{{errors[0]}}</p>
                </ValidationProvider>
            </div>
                ...
                if (this.date1 === 'shipping_date_from') {
                    ...
                    if (d < d1) {
                        this.$store.dispatch('editNewListing', {
                            key: 'unshipping_date_from',
                            value: this.listing[this.date1]
                        });
                    }
                    if (d0 < d1) {
                        this.$store.dispatch('editNewListing', {
                            key: 'unshipping_date_to',
                            value: this.listing[this.date1]
                        });
                    }
                }
