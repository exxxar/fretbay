<template>
    <div>
        <div class="row d-flex">
            <div class="col-md-4">
                <a class="btn btn-link p-0" href="/find-transporter"><i class="fas fa-dolly"></i> Try to
                    <strong>{{$trans('profile.page.find_a_transporter')}}</strong></a>
            </div>
        </div>
        <p v-if="listings.length>0">{{$trans('profile.page.count_results')}}: {{listings.length}} <i class="fas fa-boxes"></i></p>
        <hr>
        ...
        <h4 class="text-center" v-if="listings.length===0">{{$trans('profile.page.no_listings')}}</h4>
