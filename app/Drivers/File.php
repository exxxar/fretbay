...
//            throw new LanguageExistsException(__('translation::errors.language_exists', ['language' => $language]));
            throw new LanguageExistsException('Такой язык уже существует');
        }
...
//            }
    ...
    /**
     * Sync all translations keys in lang files.
     *
     * @return void
     */
    public function syncLangFiles() {
        $records = [];
        ...
        $temp_groups = collect([]);
        $keys_by_groups = (object)[];

        foreach ($languages as $language) {
            $obj = (object)[];
            $obj->language = $language;
            $obj->translations = $this->getGroupFilesFor($language)->mapWithKeys(function ($group) use ($keys_by_groups, $temp_groups) {
                $arr = Arr::dot($this->disk->getRequire($group->getPathname()));
                $group_name = $group->getBasename('.php');
                $temp_groups->push($group_name);
                if(property_exists($keys_by_groups, $group_name)==false)
                {
                    $keys_by_groups->{$group_name} = collect([]);
                }
                if ( count($arr) ){
                    $collection = new Collection($arr);
                    $keys_by_groups->{$group_name} = $keys_by_groups->{$group_name}->merge($collection->keys());

                    return [$group_name => $collection];
                }
                return [$group_name => new Collection([])];
            });
            array_push($records, $obj);
        }

        $groups = $temp_groups->unique()->values();
        foreach ($records as $record) {
            foreach ($groups as $group) {
                if (!$record->translations->keys()->contains($group)) {
                    $record->translations->put($group, collect());
                }
                $keys = $keys_by_groups->{$group}->unique()->values();
                foreach ($keys as $key) {
                    if (!$record->translations->get($group, collect())->keys()->contains($key)) {
                        $record->translations->get($group, collect())->put($key, '');
                    }
                }
                $this->saveGroupTranslations($record->language, $group, $record->translations->get($group));
            }
        }
    }

    /**
     * Get all translations by groups for all languages.
     *
     * @return Collection
     */
    public function getAllTranslations() {
        ...
        $language_translations = $first->translations;

        return $language_translations->map(function ($translations, $group) use ($language, $records) {
            $translations = $translations->toArray();
            array_walk($translations, function (&$value, &$key) use ($group, $language, $records) {
                $value = [
                    $language => $value,
                ];
                foreach ($records as $record){
                    if($record->translations->isNotEmpty()) {
                        $value[$record->language] = $record->translations->get($group, collect())->get($key);
                    }
                    else {
